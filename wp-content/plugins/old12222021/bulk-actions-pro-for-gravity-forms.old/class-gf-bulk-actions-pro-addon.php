	...
	public function scripts() {
		$scripts = array(
			array(
				...
				'deps'    => array('jquery', 'jquery-ui-core', 'jquery-ui-draggable', 'jquery-ui-sortable', 'thickbox', 'gform_chosen'),
				'strings' => array(
					...
					'fieldVisibilityVisible' => esc_html__('Visible', 'gf_bulk_actions_pro'),
					'fieldVisibilityHidden' => esc_html__('Hidden', 'gf_bulk_actions_pro'),
					'fieldVisibilityAdministrative' => esc_html__('Administrative', 'gf_bulk_actions_pro'),
				),
				'enqueue' => array(
					array(
						...
						<td>
							<select name="gform_id" id="gform_select">
								...
								foreach($forms as $form):
                                    if ( is_callable('fg_advancedpermissions') ) {
                                        $caps = fg_advancedpermissions()->get_capabilities_for_form( $form['id'] );
	                                    if ( !empty($caps) && isset($caps['gravityforms_edit_forms_fields']) && empty($caps['gravityforms_edit_forms_fields']) ) {
		                                    continue;
	                                    }
                                    }
                                    ?>
			...
			<div id="form_fields_wrap">
				...
				<form id="gf_bulk_actions_fields_edit" method="post" action="<?php echo self_admin_url('admin.php?page='.$this->_slug); ?>" novalidate="novalidate">
					...
					<table id="bulk_fields_edit" class="form-table">
						...
						<tr valign="top">
							...
							<th style="width:20%;"><?php _e('Field Description', 'gf_bulk_actions_pro'); ?></th>
							<th style="width:15%;"><?php _e('Admin Label', 'gf_bulk_actions_pro'); ?></th>
							<th style="width:15%;"><?php _e('CSS Class', 'gf_bulk_actions_pro'); ?></th>
							<th style="width:20%;"><?php _e('Visibility', 'gf_bulk_actions_pro'); ?></th>
						...
						<tr valign="top" class="bulk">
							...
							<td align="center">
								<select id="bulk_fields_visibility" name="bulk_fields_visibility">
									<option value="unchanged"><?php _e('Leave unchanged', 'gf_bulk_actions_pro'); ?></option>
									<option value="visible"><?php _e('All visible', 'gf_bulk_actions_pro'); ?></option>
									<option value="hidden"><?php _e('All hidden', 'gf_bulk_actions_pro'); ?></option>
									<option value="administrative"><?php _e('All administrative', 'gf_bulk_actions_pro'); ?></option>
								</select><br/>
                                &nbsp;
							</td>
							<td align="center">
				...
				<ul id="form_fields" class="fields">
					...
					foreach($fields as $field):
						...
						?>
						<li class="postbox field <?php echo $field_class_extra; ?>" id="field_<?php echo $field['id']; ?>" data-custom-cls="<?php echo $field_custom_cls; ?>" data-id="<?php echo $field['id']; ?>" data-type="<?php echo $field['type']; ?>" data-admin-label="<?php echo $field['adminLabel']; ?>" data-visibility="<?php echo $field['visibility']; ?>" data-description="<?php echo htmlspecialchars($field['description'], ENT_QUOTES, 'UTF-8'); ?>">
