	...
	extends \threewp_broadcast\premium_pack\base
{
	...
	**/
	...
	public function _construct()
	{
		...
		new Post_Action_Queue();
		new Post_Queue();

		if ( defined( 'WP_CLI' ) && WP_CLI )
		{
			$cli = new CLI();
			\WP_CLI::add_command( 'broadcast_queue', $cli );
		}
	}
	...
	public function activate()
	{
		...
		{
			/**
				@brief		Add priority column.
				@since		2019-04-23 18:54:00
			**/
			$query = "ALTER TABLE `".$this->wpdb->base_prefix."3wp_broadcast_queue_data` ADD `priority` INT NOT NULL DEFAULT '100' COMMENT 'Item priority'  AFTER `type`,  ADD   INDEX  `priority` (`priority`);";
			$this->query( $query );
			$db_ver = 3;
		}
	...
	**/
	public function admin_menu_queue()
	{
		...
		$per_page = $this->get_site_option( 'items_per_page' );
		...
		if ( $this->get_site_option( 'process_queue' ) )
			...
			$r .= $this->p( __( 'Queue AJAX processing has been disabled in the settings.', 'threewp_broadcast' ) );
	...
	**/
	public function admin_menu_settings()
	{
		...
		$items_per_page = $fs->number( 'items_per_page' )
			->description( __( 'A maximum amount of queue items to show on each page in the overview.', 'threewp_broadcast' ) )
			->label( __( 'Items per page', 'threewp_broadcast' ) )
			->min( 1 )
			->max( 1000000 )
			->value( $this->get_site_option( 'items_per_page' ) );
		...
		$locking = $fs->checkbox( 'locking' )
			->checked( $this->get_site_option( 'locking' ) )
			// Input title
			->description( __( 'If you are experiencing duplicates, and have many authors processing the queue via Ajax simultaneously, try enabling this setting that will lock the queue to one ajax process at a time.', 'threewp_broadcast' ) )
			// Input label
			->label( __( 'Locking enabled', 'threewp_broadcast' ) );
		...
		if ( $form->is_posting() )
		{
			...
			$this->update_site_option( 'items_per_page', $items_per_page->get_post_value() );
			$this->update_site_option( 'items_per_process', $items_per_process->get_post_value() );
			$this->update_site_option( 'locking', $locking->is_checked() );
	...
	**/
	...
	{
		$this->activate();	// Upgrade the DB if necessary.
	...
	/**
		@brief		Process the queue once.
		@since		2020-01-20 13:18:41
	**/
	public function http_process()
	{
		...
		// Thank you https://wordpress.org/support/topic/fix-featured-images-not-broadcasting/
		add_action( 'threewp_broadcast_broadcasting_started', function ( $action )
		{
				...
				// See https://wordpress.org/support/topic/fix-featured-images-not-broadcasting/
		...
		while ( true )
		{
			...
			{
				$this->debug( 'We have now spent %s seconds processing %s items. 25%% time remains, so best we continue next time.',
					$time_spent,
					$items_processed
				);
				break;
	...
	**/
	public function login_head()
	{
		...
		exit;		// Don't waste server resources displaying the login window.
	}
	...
	**/
	public function threewp_broadcast_menu( $action )
	{
		...
		$access = apply_filters( 'broadcast_queue_menu_access', $access );
		if ( ! $access )
			return;
	...
	**/
	public function wp_ajax_broadcast_queue_process( $POST = '' )
	{
		...
		{
			$ajax->finished = true;
			$ajax->html( __( 'Queue is locked. Another process is busy.' . time(), 'threewp_broadcast' ) );
			$ajax->no_items = true;
			return $ajax->to_json();
		}
		...
		$started_processing_action = $this->new_action( 'started_processing' );
		$started_processing_action->post = $POST;
		$started_processing_action->execute();
		...
		foreach( $data_items as $data_item )
		{
			...
			try
			{
				...
				{
					// Partial broadcasts don't count as an attempt.
					if ( isset( $action->partial_broadcast ) )
						if ( $action->partial_broadcast )
						{
							$item->attempts--;
							$item->db_update();
						}
		...
		$finished_processing_action = $this->new_action( 'finished_processing' );
		$finished_processing_action->ajax = $ajax;
		$finished_processing_action->post = $POST;
		$finished_processing_action->execute();
	...
	/**
		@brief		Return the path of the lock file.
		@since		2020-02-06 08:50:11
	**/
	public function get_lock_file_path()
	{
		return __DIR__ . '/cron.lock';
	}

	/**
		@brief		Try to lock the queue.
		@since		2020-02-06 08:49:48
	**/
	public function lock_queue()
	{
		$locking = $this->get_site_option( 'locking' );
		if ( ! $locking )
			return true;

		// Prevent several crons from running at the same time.
		...
		// Is the lock file old?
		if ( $locked )
			if ( time() - filemtime( $lock_file_path ) > 60 )
			{
				$locked = false;
		...
		if ( $locked )
		{
			$file_pid = file_get_contents( $lock_file_path );
			if ( $file_pid == $pid )
				return true;
		}
		...
		file_put_contents( $lock_file_path, $pid );
		return true;
	}

	/**
		@brief		Unlock the queue.
		@since		2020-02-06 08:51:39
	**/
	...
	**/
	...
	{
		return md5( wp_salt( 'auth' ) . 'broadcast_queue_process' );
	}
	...
	**/
	...
	public function site_options()
	{
		return array_merge( [
			...
			'items_per_page' => 250,					// How many queue items to show per page.
			/**
				@brief		Use queue locking functionality.
				@since		2021-03-24 09:29:47
			**/
			'locking' => false,							// Use queue locking.
	...
	{
		$clean = apply_filters( 'broadcast_queue_clean_queue', true );
