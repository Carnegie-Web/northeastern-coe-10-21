...
class UploadDir {
    ...
    public function getFolderPath($folder) {
        ...
        $result = \DevOwl\RealPhysicalMedia\Util::withoutFilters('mime_types', function () use ($folder, $callable) {
            ...
            $path = \trim($folder->getPath('/', [$this, 'getFolderPathMap'], [$this, 'getFolderPathFilter']), '/\\');
            remove_filter('mime_types', $callable, \PHP_INT_MAX);
            remove_filter('upload_mimes', $callable, \PHP_INT_MAX);
            return $path;
        });
        return $result;
    }
